...
更新日期：2026-09-22。本文描述当前场景 Tilemap 导航与单位执行合同。
...
flowchart TD
    ...
    Search --> Map
    AI[UnitMeleeAILogic：选择敌人、判断攻击范围] --> Request[UnitNavigationComp：目的地与请求版本]
...
- **AI 决定去哪里。** 进入技能攻击范围后取消导航，再尝试攻击；冷却中仍在范围内等待。
...
GameBoot 创建 NavigationSystem，UnitEntity 直接装配单位侧对象。每个实体内按 Command → Navigation → Movement 更新；这不是全场跨实体的阶段调度，物理速度仍在现有 Update 链路赋值。
...
| Arrived | 输出零方向；目标移动至少 0.5 世界单位或开始新请求后才能重新规划。 |
| Blocked | 输出零方向；与 Arrived 相同，不针对静态失败定时重试。 |
...
`INavigationSystem.FindPath` 只在主线程同步串行调用。输出 List 归调用者持有，服务不保存其引用。
...
所有失败结果的输出路径为空。缺少输出列表时优先返回 InvalidRequest；输出列表有效但无地图时返回 MapUnavailable。
...
- 不提供运行时 Tile 变更同步、动态障碍占格、单位避让、编队、流场或异步请求调度。额外障碍 Prefab 不会自动加入地图。
...
- 关卡生命周期：[DemoState](../Scripts/GameLogic/GameFlow/States/DemoState.cs)。
...
- 上下游：[UnitMeleeAILogic](../Scripts/GameLogic/Units/Entities/Logic/UnitMeleeAILogic.cs)、[UnitMovementLogic](../Scripts/GameLogic/Units/Entities/Logic/UnitMovementLogic.cs)、[UnitPhysicsComp](../Scripts/GameLogic/Units/Entities/Components/UnitPhysicsComp.cs)。
